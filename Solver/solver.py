...
class Solver:
    ...
    def get_ignore_string(self, node):
    ...
    def expand_node(self, node, id: int):
        ...
        if node.parent.id == 0 and node.child and node.child.id == 0:
            return

        ignore_str = self.get_ignore_string(node)
        if node.num_children > 0:
            result = self.engine.evaluate(node, ignore=ignore_str)
        else:
            result = self.engine.evaluate(node)

        self.tree.tree_expand(node, result, id)
    ...
    def solve(self, simulations: int = 100):
        ...
        while simulation_step < simulations:
            ...
            self.expand_node(leaf, simulation_step + 1)
            print("expand another")
            par = previous_turn_node(leaf)
